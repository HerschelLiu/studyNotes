...
const store = new Vuex.Store({
  ...
  getters: {
    doneTodos: state => {
        return state.todos.filter(todo => todo.done) //通过方法访问 返回done属性值为true的
    }，
    //  Getter 也可以接受其他 getter 作为第二个参数：
...
```js
名字(state) {// 第一个默认为state，接下来的为自定义参数。
           state.count++
}
```
...
<template>
  <div>
      <div class="top" style="margin-bottom: 10px;">
          <div>{{msg}}</div>
        <button @click="commitStore">change</button>
      </div>
      <div class="bot">
...
export default {
  ...
  computed: {
    msg: {
        ...
        set(value) {
            console.log(value);
            this.$store.commit('setCount2', value);
        }
    }
  },
  methods: {
...
**<font color=red>注：computed里直接写数据的get，set方法的话，data（）{}里不需要写上，不然会warning：“Duplicated key 'msg'”，err：“The computed property "msg" is already defined in data.”</font>**

## 数据持久化

其实本质上就是用localStorage、cookie、sessionStorage存储数据
...
注：如果报错，是因为在运行的时候添加了数据持久化，只要停止，然后重新`npm run dev`就可以了