...
export function useDate() {
  const realTime = useStorage<TimeStamp>('timeStamp', '{"local":0,"server":0}') // 没有获取服务器时间的接口就没用
...
export function useDay(date: number | string | undefined | Date, format = '星期D'): string {
...
export function useDistanceTime(millisecond: number, format = 'h:m:s'): string {
  ...
  const day: number | string = Math.floor(time / 60 / 60 / 24)
