...
* require.js是模块化要引入的文件
* main.js是配置文件
* **注意**:<font color="red">data-main这个名字不能变,后面引用的文件可以改名字,比如`data-main="./js/123.js"`</font>
...
require(['jquery', './js/dianji'], function(
    $,
    dianji,
    slider,
    wraper,
...
* paths是引用的路径,其中jquery是 自定义引用文件的别名,后面的值是路径
* require()中的是引用的文件,也相当于实参,也相当于路径,后面function中,按前面的顺序,起相应的形参名
* 这里有两种写法

```js
// 1.直接require调用
require(['jquery', './js/dianji'], function($,dianji)
// 2.paths里引路径,require()调用
require.config({
    paths: {
        jquery: 'jquery.min',
        dianji: './js/dianji'
...
require(['jquery', 'dianji'], function($, dianji) {})
```
```js
...
* 使用define调用js
* []中写的是require中配置好的js
* 后面的function括号里相当于给引用的js起名,好调用方法
* 在define的function中写好方法,在require的function中调用方法,如下

```js
// 模块 dianji.js
define(['jquery'], function($) { // 最外层都是这个
    return {
        all: function() { // 相应的方法
            // 代码
        }
