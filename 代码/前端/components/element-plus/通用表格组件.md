...
<template>
  <div class="comp-table">
    ...
    <TableToolbar
      ...
      :show-column-setting="showTableToolBar"
...
<script lang="ts" setup generic="T extends Record<string, any>, R extends Record<string, any>">
  ...
  import { useListRef } from '@/hooks/useList'
  import { useTableHeight } from '@/hooks/useTable'
  import { useTempRefs } from '@/hooks/useTempRefs'

  import TableToolbar from '@/components/TableToolbar/index.vue'

  interface Prop extends DefineListProps<T, R>, DefineCheckAllProps<R> {
    /** 列配置 */
  ...
  interface Emits extends DefineListEmits<T, R>, DefineCheckAllEmits<R> {
    /** 刷新按钮点击 */
    queryTable: []
    /** 搜索区显隐切换 */
  ...
  interface Slots {
    toolbar: () => any
    [name: `col-${string}`]: (props: { row: R; scope: CellScope }) => any
  }
  defineSlots<Slots>()
  ...
  /** 单元格格式化 */
  const formatCell = (row: R, col: TableColumn<R>) => {
    const value = (row ?? ({} as R))[col.prop as keyof R] as string | number
    const format = col.format
    if (!format) {
      return useFormatValue(value)
    }
    if (format.type === 'enum') {
      return useFormatValue(value, 'enum', format.formatStr)
    }
    return useFormatValue(value, format.type, format.formatStr)
  }
</script>
...
```ts
import { EnumObject } from '@/hooks/useFormatValue'

/** 列格式化配置（驱动 useFormatValue） */
export type TableColumnFormat = { type: 'boolean' | 'date'; formatStr?: string } | { type: 'enum'; formatStr: EnumObject }
...
export interface TableColumn<T> {
  ...
  slot?: boolean
  /** useFormatValue 格式化配置；与 slot 互斥 */
