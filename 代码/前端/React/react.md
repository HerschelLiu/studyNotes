...
它可以帮助我们更好地管理组件的状态。它的作用类似于Redux中的Reducer，可以将组件的状态和操作封装在一起，使得状态的变化更加可控和可预测。

使用useReducer需要传入两个参数：reducer函数和初始状态。reducer函数接收两个参数：当前状态和操作，返回一个新的状态。初始状态可以是任何类型的值，例如对象、数组、字符串等。

下面是一个简单的示例，演示如何使用useReducer来管理组件的状态：
...
在上面的示例中，我们定义了一个reducer函数，它接收当前状态和操作，根据操作的类型返回一个新的状态。然后在Counter组件中使用useReducer来管理组件的状态，初始状态为{ count: 0 }。在组件中可以通过dispatch函数来触发操作，从而改变组件的状态。

使用useReducer可以让我们更好地管理组件的状态，使得状态的变化更加可控和可预测。它适用于一些复杂的状态管理场景，例如表单、列表等。但是需要注意的是，使用useReducer也会增加代码的复杂度，需要谨慎使用。
...
- timeoutMs：超时时间，单位为毫秒。如果过渡时间超过了这个时间，React将会中止过渡并抛出一个错误。

- busyDelayMs：延迟时间，单位为毫秒。在过渡开始之前，React将会等待这个时间，以确保过渡不会太快而导致闪烁。
...
> Vite 对 .scss, .sass, .less, .styl 和 .stylus 文件的提供了内置支持。没有必要为它们安装特定的 Vite 插件，但必须安装相应的预处理器依赖。
