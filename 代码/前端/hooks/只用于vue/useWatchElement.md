 ...
 */
const useWatchElement = <T extends Element>(event: 'alway' | 'enter' | 'leave', selector: () => T | null, callback: Function, immediate = false) => {
  // 初始调用
  let target = selector()
  ...
  const onMutation: MutationCallback = mutations => {
    ...
    mutations
      ...
      .some(node => node === target || node.contains(target)) && callback(target)
  }
