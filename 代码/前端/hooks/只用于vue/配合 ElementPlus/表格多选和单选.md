  ...
  >
    <el-table-column label="选择" width="100" align="center">
      ...
      <template #default="{ row }">
        <el-checkbox v-if="multiple" v-model="row.checked" :disabled="row.disabled" @change="emit('checked', row)" />
        <el-radio
          v-else
...
export const useTableSelection = (props, list) => {
  const _list = isRef(list) ? list.value : { ...list };
  ...
  watch(
    ...
    (newVal) => {
      ...
      isSelectAll.value = filterArr.every((item) => item.checked);
