...
**需搭配ElementPlus的ElDatePicker组件**
  ...
  */
 export const defineDatePickerShortcuts = (isFuture = false) => {
   const now = +useDate()
  ...
  * @param isFuture false表示将要选择过往的时间，true表示将要选择未来的时间
  */
 export const defineDatePickerShortcut = (isFuture = false) => {
   const now = +useDate()
   const [Y, M, D] = useDateFormat(now, 'Y-M-D').split('-')
   const [h, m, s] = useDateFormat(now, 'h:m:s').split(':')
   let array: PickerConfig[] = []
 ...
 /** 日期区间-search */
 export default (startTime: Ref<Key>, endTime: Ref<Key>) => {
 ...
 /** 日期区间-表单页 */
 export function useDatePickerByForm(form: Record<string, any>, startKey: string, endKey: string) {
   const rangeDate = computed({
     get() {
