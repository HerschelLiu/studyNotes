...
```bash
npm install eslint eslint-config-prettier eslint-plugin-import eslint-plugin-node eslint-plugin-prettier eslint-plugin-vue @eslint/js typescript-eslint vue-eslint-parser --save-dev
```
...
若使用 `unplugin-auto-import`，需在 Vite 配置中 **排除 ESLint 插件冲突**
