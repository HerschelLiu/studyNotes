```vue
<template>
    <view class="con" :style="custom ? 'padding: 0;' : ''">
        <movable-area class="area" :style="{ height: areaHeight }" @mouseenter="mouseenter" @mouseleave="mouseleave">
            ...
            <block v-else-if="imgType == 1 || imgType == 2">
                <view class="add" v-if="imageList.length < number" :style="{ top: add.y, left: add.x, width: viewWidth + 'px', height: viewWidth + 'px' }" @click="addImages">
                    <view class="add-wrap" :style="{ width: childWidth, height: childWidth, backgroundColor: custom ? 'rgba(0, 0, 0, 0)' : '#eeeeee' }">
                        <image style="width: 100%;height: 100%;" :src="src + 'addImg.png'" v-if="imgType == 1"></image>
                        <image style="width: 100%;height: 100%;" :src="src + 'imgUpLoad.png'" v-if="imgType == 2"></image>
                    </view>
                </view>
            </block>
            <block v-else>
...
export default {
    ...
    props: {
        ...
        // 以下为自己修改部分
        delBtnType: {
            // 图片删除按钮样式 1 默认 2 订制需求
            type: [String, Number],
            default: 1
        },
        customAuto: {
            // custom需为true，为了兼容之前的写法（上传图片的逻辑在这个组件写的）
            type: Boolean,
            default: true
        },
        customPreview: {
    ...
    methods: {
            ...
            if (this.custom) {
                if (this.customAuto) {
                    ...
                    uni.chooseImage({
                        ...
                        success: res => {
                            this.uploadFileCheck(checkNumber, res);
                        }
                    });
                } else {
                    this.$emit('addImage');
        ...
        uploadFileCheck(checkNumber, res) {
            // 自己修改的
            ...
            Promise.all(
                ...
                data.forEach((item, index) => {
                    ...
                    if (_result.status == 200) {
                        this.addProperties(imgPathArr[index], _res.pic_id);
                    } else {
                        errImgNum++;
                    }
                });
        ...
        sortList() {
            ...
            for (let i = 0; i < list.length; i++) {
                list[i] = {
                    src: list[i].src,
                    pic_id: list[i].pic_id
                };
            }
            this.$emit('update:list', list);
        },
        addProperties(src, pic_id) {
        ...
        imgPreview(index) {
            ...
            if (this.customPreview) {
                this.$emit('customImgPreview', { index });
                return;
            }
            let imgsArray = [];
            for (let i = 0; i < this.imageList.length; i++) {
                imgsArray.push(this.imageList[i].src);
            }
            // #ifdef APP-PLUS
            uni.previewImage({
                current: index,
